...
(0, _mocha.describe)('isValidNumber', function () {
  (0, _mocha.it)('should validate phone numbers', function () {
    (0, _chai.expect)(isValidNumber('+1-213-373-4253')).to.equal(true);
    (0, _chai.expect)(isValidNumber('+1-213-373')).to.equal(false);
    (0, _chai.expect)(isValidNumber('+1-213-373-4253', undefined)).to.equal(true);
    (0, _chai.expect)(isValidNumber('(213) 373-4253', 'US')).to.equal(true);
    (0, _chai.expect)(isValidNumber('(213) 37', 'US')).to.equal(false);
    (0, _chai.expect)(isValidNumber({
      country: 'US',
      phone: '2133734253'
    })).to.equal(true);

    // No "types" info: should return `true`.
    (0, _chai.expect)(isValidNumber('+380972423740')).to.equal(true);
    (0, _chai.expect)(isValidNumber('0912345678', 'TW')).to.equal(true);
    ...
    (0, _chai.expect)(isValidNumber('07624369230', 'GB')).to.equal(true);
  });
  (0, _mocha.it)('should refine phone number validation in case extended regular expressions are set for a country', function () {
    // Germany general validation must pass
    (0, _chai.expect)(isValidNumber('961111111', 'UZ')).to.equal(true);
    ...
    (0, _chai.expect)(isValidNumber('961111111', 'UZ')).to.equal(false);

    // Extended validation must pass for a valid phone number
    (0, _chai.expect)(isValidNumber('912345678', 'UZ')).to.equal(true);
  ...
  (0, _mocha.it)('should work in edge cases', function () {
    ...
    (0, _chai.expect)(isValidNumber('+499821958a')).to.equal(false);
    (0, _chai.expect)(isValidNumber('88005553535x', 'RU')).to.equal(false);
    ...
    (0, _chai.expect)(isValidNumber('+3725555555')).to.equal(true);
  ...
  (0, _mocha.it)('should accept phone number extensions', function () {
    // International
    (0, _chai.expect)(isValidNumber('+12133734253 ext. 123')).to.equal(true);
    // National
    (0, _chai.expect)(isValidNumber('88005553535 x123', 'RU')).to.equal(true);
  });
});
